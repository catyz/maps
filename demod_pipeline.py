...
#     obs.detectors = sa_tpt.get_dets_by_obsID(obs)
    obs.load_metadata()

sa_data.all_requested_dets = sa_tpt.get_all_requested_dets(sa_data)
...
sa_operator_stack = sa_pf.OperatorComposite(
    sa_hwp.HWPSignalFilter(demod_modes=[0, 4], decimate_factor=1, use_toast=False),
...
def save_tod(data):
    tod = data.obs[0]['tod']
    np.save('demod0', tod.cache[f'data_demod0_{obs.detectors[0]}'])
    np.save('demod4r', tod.cache[f'data_demod4r_{obs.detectors[0]}'])
    np.save('demod4i', tod.cache[f'data_demod4i_{obs.detectors[0]}'])
...
tpt.apply_mapmaker(args, comm, data, 'maps_demod', 'data', nnz=3, bin_only=True) 
